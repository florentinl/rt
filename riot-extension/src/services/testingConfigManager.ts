    ...
    try {
      await Promise.all([
        ...
        config.update(
          ...
          pytestArgs ? true : undefined,
          target,
        ),
        config.update(
          "testing.unittestEnabled",
          pytestArgs ? false : undefined,
          target,
        ),
      ]);
      ...
      if (pytestArgs) {
        this.logLine(
          `Updated pytest configuration for ${workspaceFolder.fsPath}: ${pytestArgs.join(" ")}`,
        );
      } else {
        this.logLine(
          `Cleared pytest configuration for ${workspaceFolder.fsPath}`,
        );
      }
    } catch (err) {
