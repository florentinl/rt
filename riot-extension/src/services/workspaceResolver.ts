...
export class WorkspaceResolver {
    ...
    getWorkspaceFolder(
        ...
        if (environment) {
            const folder = vscode.workspace.getWorkspaceFolder(environment.environmentPath);
            if (folder) {
                return folder.uri;
            }
        }
     ...
     */
    getWorkspaceFoldersForScope(scope: GetEnvironmentsScope | RefreshEnvironmentsScope): vscode.Uri[] {
        if (scope === 'global') {
            return [];
        }
     ...
     */
    private extractUri(scope?: SetEnvironmentScope | GetEnvironmentScope | vscode.Uri): vscode.Uri | undefined {
        if (isUri(scope)) {
            return scope;
        }
