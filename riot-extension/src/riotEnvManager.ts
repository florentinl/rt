...
export class RiotEnvManager implements EnvironmentManager {
    ...
    async set(scope: SetEnvironmentScope, environment?: PythonEnvironment): Promise<void> {
        const folder = this.workspaceResolver.getWorkspaceFolder(scope, environment);
        if (!folder) {
            if (environment) {
     ...
     */
    async forceReinstallEnvironment(scope: GetEnvironmentScope, environment: PythonEnvironment): Promise<void> {
        const folder = this.workspaceResolver.getWorkspaceFolder(scope, environment);
    ...
    buildEnvironment(venv: RtVenv, ctx: RtExecutionContext, workspaceRoot?: string): PythonEnvironment {
        ...
        const { displayName, shortDisplayName } = this.formatter.buildDisplayNames(venv, ctx);
        const displayPath = workspaceRoot && path.isAbsolute(ctx.venv_path)
        ...
        return {
            ...
            execInfo: {
                ...
                activation: process.platform === 'win32'
                    ? [{ executable: activate }]
                    : [{ executable: 'source', args: [activate] }],
                deactivation: process.platform === 'win32'
    ...
    private async fetchEnvironmentsForFolders(folders: vscode.Uri[]): Promise<PythonEnvironment[]> {
        const results = await Promise.all(folders.map(f => this.fetchEnvironments(f.fsPath)));
        return results.flat();
    }
        ...
        try {
            await vscode.window.withProgress(
                ...
                async (progress) => {
                    ...
                    const ctx = venv?.execution_contexts.find(c => c.hash === environment.envId.id);
                    await this.testingConfigManager.updateConfiguration(folder, ctx);

                    this.currentEnvironment.set(cwd, environment);
                    await this.statePersistence.saveLastEnvironment(cwd, environment.envId.id);
                    this.envEmitter.fire({ uri: folder, old: previous, new: environment });
                },
            );
        } catch (err) {
            if (err instanceof vscode.CancellationError) {
                this.log?.info(`Environment activation cancelled for ${environment.displayName}`);
                return;
            }
            throw err;
        } finally {
    ...
    private async restoreEnvironment(cwd: string): Promise<PythonEnvironment | undefined> {
        const savedEnvId = this.statePersistence.getLastEnvironment(cwd);
        if (!savedEnvId) {return undefined;}
    ...
    private findVenvByHash(venvs: RtVenv[], hash: string): RtVenv | undefined {
        for (const venv of venvs) {
            if (venv.hash === hash) {return venv;}
            if (venv.execution_contexts?.some(ctx => ctx.hash === hash)) {return venv;}
        }
        return undefined;
    }
