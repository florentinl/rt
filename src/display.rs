use crossterm::{
    ExecutableCommand, QueueableCommand, SynchronizedUpdate, cursor,
    style::{Color, Stylize},
...
/// Strip ANSI escape codes from a string, returning only visible text.
...
/// Measure the visual width of a string (ignoring ANSI escape codes).
...
impl BuildStep {
    ...
    /// Append multiple output lines to this step's buffer.
    pub fn append_outputs(&mut self, lines: Vec<String>) {
        for line in lines {
...
impl DisplayManager {
    ...
    pub fn new() -> io::Result<Self> {
        ...
        let display = Self {
            ...
            render_signal: Arc::new(RenderSignal::default()),
    ...
    pub fn register_step(&self, id: &str, description: &str) {
        ...
        // Idempotent: don't reset a step that was already registered.
        if !steps.contains_key(id) {
            steps.insert(id.to_string(), BuildStep::new(description.to_string()));
            drop(steps);
            self.request_render();
        }
    }
    ...
    /// Append multiple output lines to a build step.
    ///
    /// # Panics
    ///
    /// Panics if the internal mutex is poisoned.
    pub fn append_outputs(&self, id: &str, lines: Vec<String>) {
        if lines.is_empty() {
            return;
        }
        ...
        let mut steps = self.steps.lock().unwrap();
        if let Some(step) = steps.get_mut(id) {
            // Store the full lines - truncation will happen during rendering
            // based on current terminal width to support terminal resizing.
            step.append_outputs(lines);
            drop(steps);
            self.request_render();
        }
    }
    ...
    pub fn start_refresh_loop(&self) {
        ...
        let render_signal = Arc::clone(&self.render_signal);
        let min_render_interval = self.min_render_interval;
        let heartbeat_interval = self.heartbeat_interval;
        let start_time = self.start_time;

        let handle = thread::spawn(move || {
            let mut next_render_allowed = Instant::now();
            let mut next_heartbeat = Instant::now() + heartbeat_interval;
            let mut last_rendered = RenderSnapshot::capture(start_time);
            let mut heartbeat_pending = false;
            let mut lines_last_render: usize = 0;

            loop {
                let (dirty, shutting_down) = {
                    let mut state = render_signal.state.lock().unwrap();
                    let pending_render = state.dirty || heartbeat_pending;
                    let wake_deadline = if pending_render {
                        next_render_allowed.min(next_heartbeat)
                    } else {
                        next_heartbeat
                    };
                    let timeout = wake_deadline.saturating_duration_since(Instant::now());
                    let (guard, _) = render_signal.cv.wait_timeout(state, timeout).unwrap();
                    state = guard;
                    let dirty = state.dirty;
                    if dirty {
                        state.dirty = false;
                    }
                    (dirty, state.shutdown)
                };

                let now = Instant::now();
                if now >= next_heartbeat {
                    heartbeat_pending = RenderSnapshot::capture(start_time) != last_rendered;
                    next_heartbeat = now + heartbeat_interval;
                }

                let should_render =
                    shutting_down || (now >= next_render_allowed && (dirty || heartbeat_pending));

                if should_render {
                    let steps = steps.lock().unwrap();
                    let mut stderr = stderr.lock().unwrap();
                    if let Err(e) =
                        Self::render_locked(&steps, &mut stderr, &mut lines_last_render, start_time)
                    {
                        eprintln!("Display render error: {e}");
                    } else {
                        last_rendered = RenderSnapshot::capture(start_time);
                        heartbeat_pending = false;
                    }
                    next_render_allowed = Instant::now() + min_render_interval;
                }
    ...
    fn request_render(&self) {
        let mut state = self.render_signal.state.lock().unwrap();
        if state.dirty {
            return;
        }
        state.dirty = true;
        drop(state);
        self.render_signal.cv.notify_one();
    }
    ...
    fn render_locked(
        ...
        let rendered_lines = stderr.sync_update(|stderr| {
            Self::rewind_cursor(stderr, *lines_last_render)?;
            stderr.queue(crossterm_terminal::Clear(ClearType::FromCursorDown))?;
            Self::render_frame(&visible_steps, stderr, &summary_line, mode)
        })??;
    ...
    fn elapsed_seconds(start_time: Instant) -> u64 {
        Instant::now()
...
impl RenderSnapshot {
    fn capture(start_time: Instant) -> Self {
        Self {
            terminal_size: DisplayManager::terminal_dimensions().ok(),
            elapsed_seconds: DisplayManager::elapsed_seconds(start_time),
        }
    }
}
