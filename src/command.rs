...
fn uv_bin() -> std::ffi::OsString {
    std::env::var_os("_RT_UV_BIN").unwrap_or_else(|| std::ffi::OsString::from("uv"))
}
...
impl ManagedCommand {
    ...
    pub fn status(mut self) -> io::Result<ExitStatus> {
        if self.sink.output_policy() == OutputPolicy::Inherit {
